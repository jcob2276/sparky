...
export async function runMonthlyRecap(
  ...
  apiKey: string,
  monthStart: string,
): Promise<{ phase1: { narrative: string; longterm_motif: string | null; question: string; theme_suggestion: string | null } }> {
  ...
  const reviews = (reviewsRes.data ?? []) as WeeklyReviewRow[];
  const wins = (winsRes.data ?? []) as DailyWinResultRow[];
  const kpis = (kpiRes.data ?? []) as KpiEntryRow[];
  const stream = (streamRes.data ?? []) as RecapStreamRow[];
  const oura = (ouraRes.data ?? []) as OuraDailyTrendRow[];
  const strava = (stravaRes.data ?? []) as StravaActivityRow[];
  const reconc = (reconcRes.data ?? []) as ReconciliationRow[];
  const patterns = (patternsRes.data ?? []) as BehavioralPatternRow[];
  const projects = (projectsRes.data ?? []) as RecapProjectRow[];
  ...
  const totalKm = runs.reduce((s: number, r: StravaActivityRow) => s + (r.distance || 0), 0) / 1000;
  const stravaLine = [
  ...
  const voiceBlock = voiceEntries.slice(0, 30).map((v) => {
    const d = new Intl.DateTimeFormat("pl-PL", { timeZone: "Europe/Warsaw", day: "numeric", month: "short", weekday: "short" }).format(new Date(v.timestamp));
  ...
  const habitsBlock = habitLogs.length
    ? habitLogs.map((l) => {
        const t = l.logged_at ? new Intl.DateTimeFormat("pl-PL", { timeZone: "Europe/Warsaw", day: "numeric", month: "short", weekday: "short" }).format(new Date(l.logged_at)) : l.date;
...
Zwróć TYLKO JSON: {"narrative": "...", "longterm_motif": "..." | null, "question": "...", "theme_suggestion": "..."}`;
  ...
  const mergedRecap = { ...((existing?.ai_recap as Record<string, unknown> | null) ?? {}), phase1 };
