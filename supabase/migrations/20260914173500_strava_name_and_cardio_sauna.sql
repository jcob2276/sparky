-- Include name and elapsed_time in strava array returned by get_desktop_dashboard_data
-- so Garmin Cardio activities can be recognized as Sauna in dashboard & Heatmap.
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'strava', (
      SELECT COALESCE(jsonb_agg(sub), '[]'::jsonb)
      FROM (
        SELECT name, sport_type, distance, moving_time, elapsed_time, start_date, best_efforts, hr_avg
