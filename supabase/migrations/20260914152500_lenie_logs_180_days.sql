-- Extend lenieLogs from 60 days to 180 days so that behavioral patterns,
-- hourly histograms, and trigger correlations have statistically robust depth.
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'strava', (
      SELECT COALESCE(jsonb_agg(sub), '[]'::jsonb)
      FROM (
        SELECT sport_type, distance, moving_time, start_date, best_efforts, hr_avg
    ...
    'lenieLogs', (
      ...
      FROM (
        -- 180-day window provides full history for robust hourly & pattern analytics
