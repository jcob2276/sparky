    ...
    <Panel title="Sen & HRV — 14 dni">
      ...
      <div className="flex gap-4 mt-2">
        <span className="flex items-center gap-1.5 text-2xs text-text-muted">
          <span className="w-3 h-[var(--ds-h-2px)] inline-block rounded" style={{ backgroundColor: C.indigo }} /> Sen (h)
        </span>
        <span className="flex items-center gap-1.5 text-2xs text-text-muted">
          <span className="w-3 h-[var(--ds-h-2px)] inline-block rounded" style={{ backgroundColor: C.emerald }} /> HRV (ms)
        </span>
      </div>
    </Panel>
  );
...
export function NutritionTrendChart({
  ...
  return (
    <Panel title="Żywienie — 14 dni">
      ...
      <div className="flex gap-4 mt-2">
        <span className="flex items-center gap-1.5 text-2xs text-text-muted">
          <span className="w-3 h-[var(--ds-h-2px)] inline-block rounded" style={{ backgroundColor: C.rose }} /> Kcal
        </span>
        <span className="flex items-center gap-1.5 text-2xs text-text-muted">
          <span className="w-3 h-[var(--ds-h-2px)] inline-block rounded" style={{ backgroundColor: C.sky }} /> Białko (g)
        </span>
      </div>
    </Panel>
  );
  ...
  return (
    <section id="biometria" className="scroll-mt-28 space-y-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <SleepHrvChart sleepData={sleepData} grid={grid} tick={tick} />
        <TrainingVolumeChart volData={volData} grid={grid} tick={tick} />
        <NutritionTrendChart nutrData={nutrData} grid={grid} tick={tick} />
      </div>
    </section>
  );
}
