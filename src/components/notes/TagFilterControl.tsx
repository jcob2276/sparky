  ...
  return (
    <div className="keep-tag-filter" aria-label="Filtr tagów">
      <div className="keep-tag-filter-mode">
        <label><ControlInput type="radio" checked={value.mode === 'all'} onChange={() => onChange({ ...value, mode: 'all' })} /> Wszystkie</label>
        <label><ControlInput type="radio" checked={value.mode === 'any'} onChange={() => onChange({ ...value, mode: 'any' })} /> Dowolny</label>
      </div>
      <div className="keep-tag-filter-list">
        {tags.map(tag => (
          <div key={tag} className="keep-tag-filter-row">
            <label>
              <ControlInput type="checkbox" checked={value.tags.includes(tag)} onChange={() => toggle(tag)} />
              <span>#{tag}</span>
            </label>
