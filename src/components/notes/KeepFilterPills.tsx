...
export default function KeepFilterPills({ activeFilter, onChangeFilter, counts }: KeepFilterPillsProps) {
  return (
    <div className="flex items-center gap-1.5 overflow-x-auto py-1 px-1 no-scrollbar select-none" role="tablist">
      {PILLS.map((pill) => {
        ...
        return (
          <Pressable
            ...
            className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold whitespace-nowrap ui-interactive duration-[var(--motion-fast)] btn-press cursor-pointer ${
              isActive
