...
export default function ShutdownScoreSliders({
  dayScore,
  setDayScore,
  moodScore,
  setMoodScore,
    ...
    <div className="space-y-3.5 bg-surface-2 dark:bg-on-accent/[0.015] border border-border-custom/50 p-4 rounded-2xl">
      <div className="space-y-1.5">
        ...
        <ControlInput
          ...
          max="10"
          value={dayScore}
          onChange={(event) => setDayScore(Number(event.target.value))}
      ...
      <div className="space-y-1.5 border-t border-border-custom/30 pt-3">
        <div className="flex items-center justify-between text-xs font-bold">
          ...
          <span className="text-success font-extrabold">{moodScore}/5</span>
        </div>
        <ControlInput
          type="range"
          min="1"
