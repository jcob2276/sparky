...
tr_enc = deb_tok(tr_sub_x, truncation=True, padding=True, max_length=256)
te_enc = deb_tok(texts_exec, truncation=True, padding=True, max_length=256)
...
args = TrainingArguments(
    output_dir='/workspace/deb_phish',
    num_train_epochs=2,
...
trainer = Trainer(model=deb_model, args=args, train_dataset=tr_ds)
t0 = time.time()
trainer.train()
print(f'DeBERTa trained in {time.time()-t0:.1f}s')
...
deb_f1 = f1_score(y_exec, preds, average='macro')
print(f'DeBERTa-v3 on C-Level: FPR={fp/(fp+tn)*100:.1f}% ({fp}/50 FP) | TPR={tp/(tp+fn)*100:.1f}% ({tp}/50 TP) | F1={deb_f1:.4f}')
...
for th in [0.5, 0.65, 0.80, 0.90]:
    ...
    print(f'DeBERTa (theta={th}): FPR={fp/(fp+tn)*100:.1f}% ({fp}/50 FP) | TPR={tp/(tp+fn)*100:.1f}% ({tp}/50 TP) | F1={cal_f1:.4f}')
