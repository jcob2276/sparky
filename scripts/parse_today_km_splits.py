...
print(f"Total valid rows with sumDistance: {len(valid_rows)}")
...
for r in valid_rows:
    ...
    if dist >= km_target:
        ...
        speeds = [x.get("directSpeed") for x in seg_rows if x.get("directSpeed") is not None]
        ...
        split_info = {
            ...
            "min_elev": round(min(elevs), 1) if elevs else 0,
            "max_elev": round(max(elevs), 1) if elevs else 0
        }
        splits_data.append(split_info)

        print(f"KM {km_count:2d}: {seg_dist:5.1f}m | Czas: {dur_m:02d}:{dur_s_rem:02d} | Tempo: {pm}:{ps:02d}/km | HR: {min_hr:3.0f} - {avg_hr:5.1f} - {max_hr:3.0f} | Kad: {avg_cad:5.1f}/{max_cad:3.0f} | Elev: +{elev_gain:4.1f}m/-{elev_loss:4.1f}m")
...
if valid_rows:
    ...
    if dist - prev_dist > 5:
        ...
        print(f"KM {km_count:2d} (Finisz {seg_dist:.1f}m): Czas: {dur_m:02d}:{dur_s_rem:02d} | Tempo: {pm}:{ps:02d}/km | HR: {min_hr:3.0f} - {avg_hr:5.1f} - {max_hr:3.0f} | Kad: {avg_cad:5.1f}/{max_cad:3.0f}")

with open("tmp/today_km_splits_parsed.json", "w", encoding="utf-8") as f:
