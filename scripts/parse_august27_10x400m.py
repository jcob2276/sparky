...
start_warmup_dist = 1000.0 # Warmup is 1.0 km
...
for i in range(10):
    ...
    tr_start = rec_rows[0].get("directTimestamp", 0) if rec_rows else t_end
    tr_end = rec_rows[-1].get("directTimestamp", 0) if rec_rows else t_end
...
print("=== EXACT 10x 400M FAST REPS PARSED ===")
for r in reps_400m:
...
with open("tmp/garmin_aug27_10reps_strict.json", "w", encoding="utf-8") as f:
    json.dump(reps_400m, f, indent=2, ensure_ascii=False)

with open("tmp/garmin_aug27_10recovers_strict.json", "w", encoding="utf-8") as f:
